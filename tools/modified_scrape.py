  ...
  if response:
    try:
      ...
      if app['success'] == False:
        ...
        return None
      # Look for a specific game and find out what's happening in the processing     
      else:
        if appID == '976730':
          Log(INFO, f"Processing 'Halo: The Master Chief Collection' with appID {appID}")
          with open(f"{debug_dir}/{appID}_response.json", 'w', encoding='utf-8') as fout:
...
def ParseSteamGame(app):
  ...
  try:
    ...
    # Fix for required_age with logging
    if 'required_age' in app:
      required_age_str = str(app['required_age']).replace('+','')
      if required_age_str.isdigit():
        game['required_age'] = int(required_age_str)
      else:
        Log(WARNING, f"Unexpected value for required_age: {required_age_str}")
        game['required_age'] = 0  # Default value if the conversion fails
    else:
      game['required_age'] = 0  # Default value if key doesn't exist
    ...
    Log(INFO, f"Parsed game: {game['name']} - Release Date: {game['release_date']} - Price: {game['price']} - Platforms: Windows: {game['windows']}, Mac: {game['mac']}, Linux: {game['linux']}")
    ...
    Log(EXCEPTION, f"Error parsing game: {ex}. Traceback: {traceback.format_exc()}")
  return game


def SaveJSON(data, filename, backup = False, retries=3, delay=1):
  filename = os.path.abspath(filename)
  Log(INFO, f'Attempting to save JSON to {filename}')
  try:
    if backup == True and os.path.exists(filename):
      ...
      for _ in range(retries):
        try:
          ...
          Log(WARNING, 'File is in use, retrying in a moment...')
          time.sleep(delay)
      else:
        raise PermissionError(f'Could not access the file after {retries} attempts: {filename} -> {name}.bak')
    ...
    for _ in  range(retries):
      try:
        ...
        Log(Warning, f'File access error, retrying in a moment: {ex}')
        time.sleep(delay)
    else:
      raise OSError(f'Failed to write to the file after {retries} attempts: {filename}')
  except Exception as ex:
    Log(EXCEPTION, f'An exception of type {ex} ocurred. Traceback: {traceback.format_exc()}')
    sys.exit()
...
if __name__ == "__main__":
  ...
  existing_games = LoadJSON(args.infile) or {}
  existing_game_ids = set(existing_games.keys())
  Log(INFO, f"Loaded {len(existing_game_ids)} existing games")
